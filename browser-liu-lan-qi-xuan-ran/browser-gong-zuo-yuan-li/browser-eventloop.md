...
执行上下文是在编译阶段产生的，在什么情况下会创建执行上下文呢？ 1. 全局执行上下文 - 执行全局代码 2. 函数执行上下文 - 执行函数代码 3. eval执行上下文 - 执行eval函数
...
1. 轮询的方式 - 带来的问题
2. 定时器的设置时长不确定，太长浪费时间成本，DOM相应不及时
3. 设置时间太短，浪费无用的工作检查DOM
4. mutationEvent - 带来的问题
5. 采用观察者模式，DOM一变化，调用对应的回调更新，这种回调是同步回调
6. 一旦一次动态修改多个节点内容，就会触发多个回调，每个回调执行时间固定的话，那更新DOM的总共耗时就会变的很长，最终会导致页面性能的问题
7. mutationObserver
8. 将mutationEvent的回调方式改变成了异步，不用每次去触发异步调用，而是等多次DOM操作之后，再去触发DOM更新
9. 采用 `异步 + 微任务`策略
10. 异步解决了性能问题
11. 微任务解决了实时性的问题
...
可以看一下协程的执行图 ![&#x534F;&#x7A0B;&#x6D41;&#x7A0B;&#x56FE;](https://raw.githubusercontent.com/Wangbaoqi/blogImgs/master/nateImgs/JavaScript/ctx/event_10.png) ![&#x534F;&#x7A0B;&#x8C03;&#x7528;&#x6808;](https://raw.githubusercontent.com/Wangbaoqi/blogImgs/master/nateImgs/JavaScript/ctx/event_11.png)
